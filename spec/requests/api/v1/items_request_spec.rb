...
describe "items API" do
  ...
  it "can return the top x items ranked by total revenue generated" do
    ...
    top_x_items = JSON.parse(response.body)

    expect(top_x_items["data"][0]["id"]).to eq("#{item_1.id}")
    expect(top_x_items["data"][1]["id"]).to eq("#{item_3.id}")
    expect(top_x_items["data"][2]).to be_nil
  end
end
